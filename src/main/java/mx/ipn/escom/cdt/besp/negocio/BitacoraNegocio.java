...
public class BitacoraNegocio
{
	 ...
	 * @return Una lista de tipo bitacora con todas las restricciones del proyecto
	 */
	public List<Bitacora> getRestriccionesProyecto(int idProyecto)
	{
		//Creo listas temporales paara poder obtenes las restricciones y los registros en la bitacora del proyecto
		...
		//Obtengo todos los registros de la bitacora que corresponden al proyecto
		...
		for (Bitacora registro : registrosBitacora) {
			...
			if(registro.getIdTipoRegistro().equals(tipoRegistro.RESTRICCION) || registro.getIdTipoRegistro().equals(tipoRegistro.OBSERVACION)){
	 ...
	 * 
	 * @param ID de la restriccion
	 * @return Una lista conteniendo todos los mensajes pertenecientes a la restriccion 
	 */
	@Transactional
	public List<Bitacora> getMensajesRestriccionBitacora(int idRestriccion)
	{
		//Creo listas temporales para poder obtener los registros de la bitacora y los mensajes de la restriccion
		...
		for(Bitacora registro : registrosBitacora){
			//Si el tipo de registro es diferente de una restriccion y el ID del registro de referencia es igual al ID de la restriccion
			if(!registro.getIdTipoRegistro().equals(tipoRegistro.RESTRICCION) && !registro.getIdTipoRegistro().equals(tipoRegistro.OBSERVACION) && registro.getIdRegistroReferencia().equals(idRestriccion)){
	 ...
	 * 
	 * @param Una lista bidimensional de tipo bitacora con los registros
	 * @return Una lista bidimensioanl de tipo bitacora con los registros ordenados
	 */
	public List<List<Bitacora>> ordenaRegistrosBitacora(List<List<Bitacora>> registrosBitacoraProyecto)
	{
		//Creo una lista bidimensional temporal para almacenar los registros ordenados
		List<List<Bitacora>> registrosOrdenadosBitacoraProyecto =  new ArrayList<List<Bitacora>>();
		
		//Creo una lista temporal para poder manejar todos los mensajes de una restriccion
		List<Bitacora> coleccionRestriccion = new ArrayList<Bitacora>();

		//Creo listas bidimensionales temporales para poder almanecnar las observaciones restricciones atendidas, turnadas y no atendidas
		...
		//Por cada lista que contiene los mensajes de la restriccion dentro de los registros de la bitacora
		for(List<Bitacora> restriccion : registrosBitacoraProyecto)
		{
			//Limpio la lista temporal para poder manejar los mesajes de la restriccion
			...
			for(Bitacora mensaje : restriccion)
			{
				...
				if( (restriccion.size()-1) == restriccion.indexOf(mensaje))
				{
					...
					if(mensaje.getIdTipoRegistro().equals(tipoRegistro.RESTRICCION) || mensaje.getIdTipoRegistro().equals(tipoRegistro.ATENCION_DIRECTOR))
					{
						//Agrego la restriccion y sus mensajes en las restricciones no atendidas
						restriccionesNoAtendidas.add(coleccionRestriccion);
					}
					//Si es una restriccion turnada
					if(mensaje.getIdTipoRegistro().equals(tipoRegistro.DERIVACION))
					{
						//Agrego la restriccion y sus mensajes en las restricciones turnadas
						restriccionesTurnadas.add(coleccionRestriccion);
					}
					//Si es una atencion inmediata o atencion turnada
					if(mensaje.getIdTipoRegistro().equals(tipoRegistro.ATENCION_INMEDIATA) || mensaje.getIdTipoRegistro().equals(tipoRegistro.ATENCION_TURNADA))
					{
						//Agrego la restriccion y sus mensajes en las restricciones no atendidas
						restriccionesAtendidas.add(coleccionRestriccion);
					}
					//Si es una observacion
					if(mensaje.getIdTipoRegistro().equals(tipoRegistro.OBSERVACION))
					{
						//Agrego la restriccion y sus mensajes en las restricciones no atendidas
		...
		Collections.sort(observaciones, new Comparator <List<Bitacora>>() {
		    public int compare(List<Bitacora> b1, List<Bitacora> b2) {
		        return b1.get(b1.size()-1).getFechaRegistro().compareTo(b2.get(b2.size()-1).getFechaRegistro());
		    }
		});
		
		Collections.reverse(observaciones);
		
		Collections.sort(restriccionesNoAtendidas, new Comparator <List<Bitacora>>() {
		    public int compare(List<Bitacora> b1, List<Bitacora> b2) {
		        return b1.get(b1.size()-1).getFechaRegistro().compareTo(b2.get(b2.size()-1).getFechaRegistro());
		    }
		});
		...
		Collections.sort(restriccionesTurnadas, new Comparator <List<Bitacora>>() {
		    public int compare(List<Bitacora> b1, List<Bitacora> b2) {
		        return b1.get(b1.size()-1).getFechaRegistro().compareTo(b2.get(b2.size()-1).getFechaRegistro());
		    }
		});
		...
		Collections.sort(restriccionesAtendidas, new Comparator <List<Bitacora>>() {
		    public int compare(List<Bitacora> b1, List<Bitacora> b2) {
		        return b1.get(b1.size()-1).getFechaRegistro().compareTo(b2.get(b2.size()-1).getFechaRegistro());
		    }
		});
		...
		//En esta parte voy agregando las restricciones y sus contenidos por relevancia
		...
		//Retorno la lista bidimensional con todas las restricciones ordenadas por relevancia
	...
	{
		//Creo listas bidimensionales temporales para almacenar los registros del proyecto y despues ordenarlos
		List<List<Bitacora>> registrosBitacoraProyecto =  new ArrayList<List<Bitacora>>();
		List<List<Bitacora>> registrosOrdenadosBitacoraProyecto =  new ArrayList<List<Bitacora>>();

		//Creo listas temporales para obtener las restricciones, mensajes y datos de las restricciones del proyecto
		...
		{
			//Limpio la lista temporal donde agrego los mensajes de cada restriccion
			coleccionRestriccion=new ArrayList<Bitacora>();

			//Obtengo los mensajes de la restriccion del proyecto
			mensajesRestriccion = this.getMensajesRestriccionBitacora(restriccion.getIdRegistro());
			
			//Agrego la restriccion a la lista donde se contienen los mensajes de la restriccion
			...
			{
				//Lo agrego a la lista donde se contienen los mensajes de la restriccion
				coleccionRestriccion.add(mensaje);
			}
		...
		registrosOrdenadosBitacoraProyecto = this.ordenaRegistrosBitacora(registrosBitacoraProyecto);
	...
	/**
	 * Se obtienen todas las restricciones de los programas pertenecientes a un programa
	 * 
	 * @param ID del programa
	 * @return Se retorna una lista bidimensional con todos los registros de la bitacora del programa
	 */
	@Transactional
	public List<List<Bitacora>> getRegistrosBitacoraPrograma(int idPrograma)
	{
		...
		//Creo una lista temporal para poder manejar las estructuras y los proyectos
		List<Estructura> estructuras;
		List<Proyecto> proyectos = new ArrayList<Proyecto>();

		//Creo listas temporales para poder manejar los registros en bitacora de los proyectos y del programa
		...
		for(Proyecto proyecto : proyectos)
		{
			...
			registrosBitacoraProyecto = this.getRegistrosBitacoraProyecto(proyecto.getIdProyecto());
		...
		//Retorno todos los registros del programa dentro de la bitacora ya ordenados
